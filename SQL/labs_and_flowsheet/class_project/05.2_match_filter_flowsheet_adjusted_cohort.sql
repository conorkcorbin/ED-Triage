# match flowsheet to adjusted cohort
# on patient ID and inpatient ID
# filter to events occurring 1 hr prior to admit
select * from
  (select * 
    from traige_TE.triage_cohort_adjusted as cohort
    left join 
...
where timestamp_add(recorded_time_jittered_utc, INTERVAL 1 HOUR) <= admit_time