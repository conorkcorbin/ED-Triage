# query the lab results, filter by lab_name
# match on CSN and patient ID
# stored as traige_TE.draft_cohort_joined_labs_uid_csn
select jc_uid, cohort.pat_enc_csn_id_coded, admit_time_jittered, label,
       rit_uid, order_id_coded, lab_name,
       ord_value, ord_num_value, reference_low, reference_high, 
       reference_unit, result_in_range_yn, result_flag, 
       result_time_jittered_utc, order_time_jittered_utc, taken_time_jittered_utc,
  from conor_db.triage_cohort_draft as cohort
  left join 
    (select * from starr_datalake2018.lab_result
      where (
  ...
  on (cohort.jc_uid = labs.rit_uid 
    and cohort.pat_enc_csn_id_coded = labs.pat_enc_csn_id_coded)
